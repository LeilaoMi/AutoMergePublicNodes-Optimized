...
> **可达性方向说明**：真测在 GitHub Actions（海外机房）执行，验证的是**节点 → 海外目标**的连通性与出口地理。它**不**模拟**中国大陆 → 节点**的穿墙可达性——这一段取决于 GFW 实时封锁策略，无固定 IP 黑名单可批量比对，必须由用户在大陆环境实测。项目通过以下间接信号提升大陆可用概率：REALITY/uTLS 抗检测优先、443 端口加权、跨周期稳定性加成、出口 CN 节点剔除。
...
| 评分 | 协议 | 延迟(ms) | 来源 |
| --- | --- | --- | --- |
| 62.6 | shadowsocks | 237.9 | Au1rxx-base64 |
| 62.6 | shadowsocks | 238.1 | Au1rxx-base64 |
| 62.55 | shadowsocks | 240.3 | Au1rxx-base64 |
| 62.54 | shadowsocks | 240.4 | Au1rxx-base64 |
| 61.82 | vless | 275.4 | DeltaKronecker-all |

### Top 来源质量

| 来源 | 评分 | 测试数 | 建议 |
| --- | --- | --- | --- |
| Surfboard-tg-mixed | 0.933 | 236 | prefer |
| Au1rxx-base64 | 0.883 | 64 | prefer |
| snakem982 | 0.88 | 55 | prefer |
| mheidari-all | 0.872 | 22 | prefer |
| DeltaKronecker-all | 0.823 | 886 | prefer |
